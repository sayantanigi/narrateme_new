...
class Batch extends CI_Controller
{
    ...
    public function add_course_instructor() {
        ...
        $course = $this->db->query("SELECT * FROM sm_course WHERE course_id = '".$this->input->post('course_idd')."'")->row();
        if(!empty($course)){
        ...
        if(!empty($meeting_api)) {
            ...
            if($result['status'] == '1') {
                ...
                $response = $this->db->insert('sm_course_instructor', $data);
                if($response){
                    $this->session->set_flashdata('success', 'Data Added Successfully');
    ...
    public function instructor_list(){
        $data['schedule_list'] = $this->db->query("SELECT * FROM sm_course_instructor")->result();
		$data['title'] = "Add Instructor";
		$this->load->view('supercontrol/header', $data);
		$this->load->view('supercontrol/scheduled_course_list', $data);
		$this->load->view('supercontrol/footer');
    }
    public function instscheduleDStatus($id) {
        $table = "sm_course_instructor";
        $val = '0';
        $field_data = array(
            'status' => $val
        );
        $this->db->where('inst_id', $id);
        $result = $this->db->update($table, $field_data);
        if ($result) {
            $this->session->set_flashdata('success', 'Deactivated Successfully');
            redirect($_SERVER['HTTP_REFERER']);
        }
    }
    public function instscheduleAStatus($id) {
        $table = "sm_course_instructor";
        $val = '1';
        $field_data = array(
            'status' => $val
    ...
    public function deleteinstschedule($id) {
        $table = "sm_course_instructor";
		$this->db->where('inst_id', $id);
